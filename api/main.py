...
def write_inside_a_box(image_array, x, y, w, h, text, color=(255,255,255)):
    ...
    font_path = "api/arial.ttf"  # You can use another font file if needed
...
async def process_image(file: UploadFile, lang_from: str = 'en', lang_to: str = 'ru') -> Response:
    ...
    # Iterate over each detected text and its bounding box coordinates
    for detection in result:
        ...
        if confidence > 0.6:  # Adjust threshold as needed
            try:
                ...
                # Find dominant color and contrasting color
                ...
                # Write the translated text inside the box
                image = write_inside_a_box(image, x1, y1, w, h, translated_text, color=text_color)

                print(f"Text: {text} | Position: ({x1}, {y1}, {x2}, {y2}) | Confidence: {confidence}")
...
#
#
# async def process_image_old(file: UploadFile, lang_from: str = 'eng', lang_to: str = 'rus') -> Response:
